...
const { iId } = injectStrict(InvestmentKey)
const loading = ref(false)


//回调方法
const { func, isRefresh } = injectStrict(PopupKey)
...
const handleShowEntity = () => {
    if (ruleForm.clientId) {
        emit('showEntity', ruleForm.clientId)
    } else {
        ElMessage.warning('Please select client first')
    }
}
</script>
...
<template>
    <div class="pop-top">
        <div class="title">{{ !iId ? 'Create Investment' : 'Edit Investment' }}</div>
        <div v-loading="loading">
            <el-button type="primary" color="#4E9F1C" @click="handleAddAdmin(ruleFormRef)">
                {{ !iId ? 'New Investment' : 'Edit Investment' }}
                <el-icon>
                    <ArrowRight />
                </el-icon>
            </el-button>
    ...
    <div class="form">
        <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="200px" status-icon>
            ...
            <el-form-item label="Entity Name" prop="investmentEntityId">
                <el-select v-model="ruleForm.investmentEntityId" placeholder="Please Entity Name" filterable clearable>
                    <el-option :label="item.entityName" :value="item.id" v-for="(item, index) in entityList" :key="index" />
                </el-select>
                <el-link class="p-t-10" type="info" @click="handleShowEntity">Add new entity</el-link>
            </el-form-item>
